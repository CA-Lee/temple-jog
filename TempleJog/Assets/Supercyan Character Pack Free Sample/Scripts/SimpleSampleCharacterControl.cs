...
public class SimpleSampleCharacterControl : MonoBehaviour
{
    ...
    private KinectController _controller;
    ...
    private void Update()
    {
        ...
        if (!m_jumpInput && (Input.GetAxis("Vertical") > 0||_controller.GetAction(TempleAction.Jump)))
        {
            m_jumpInput = true;
        }
        if (!m_right && !m_left && (Input.GetKeyDown(KeyCode.D)||_controller.GetAction(TempleAction.TurnRight)))
        {
            m_right = true;
            m_move = true;
            this.Difference();
        }
        if (!m_right && !m_left && (Input.GetKeyDown(KeyCode.A)||_controller.GetAction(TempleAction.TurnLeft)))
        {
            m_left = true;
            m_move = true;
            this.Difference();
        }
        if (!m_rightmove && !m_leftmove && (Input.GetKeyDown(KeyCode.E)||_controller.GetAction(TempleAction.SlideRight)))
        {
            m_rightmove = true;
        }
        if (!m_rightmove && !m_leftmove && (Input.GetKeyDown(KeyCode.Q)||_controller.GetAction(TempleAction.SlideLeft)))
        {
            m_leftmove = true;
        }
    ...
    {
        if (moveDifference * fix - fHorizontalMovewidth > 0 && (m_right || m_left))
        {
            transform.position += ((dir % 2 == 0) ? Vector3.forward : Vector3.right) * fHorizontalMovewidth * Roadwidth * fix;
            moveDifference -= (fHorizontalMovewidth * fix);
        }
        else
        {
